#!/usr/bin/env zsh
# Git push script for macOS/Linux
# Based on __push.sh with enhanced reporting
...
commit_changes() {
    ...
    log_step "Adding and committing changes..."
...
show_push_info() {
    log_step "Push information:"
    
    echo -e "${WHITE}Branch: ${CYAN}$BRANCH${NC}"
    echo -e "${WHITE}Remote: ${CYAN}$REMOTE${NC}"
    echo -e "${WHITE}Remote URL: ${CYAN}$(git remote get-url "$REMOTE")${NC}"
    echo -e "${WHITE}Last commit: ${CYAN}$(git log -1 --oneline)${NC}"
    
    if [[ "$PUSH_TAGS" == "true" ]]; then
        local tags
        tags=$(git tag --list | tail -5 | tr '\n' ' ')
        echo -e "${WHITE}Tags: ${CYAN}$tags${NC}"
    fi
}
...
show_summary() {
    ...
    echo -e "${WHITE}Branch: ${CYAN}$BRANCH${NC}"
    echo -e "${WHITE}Remote: ${CYAN}$REMOTE${NC}"
    echo -e "${WHITE}Force Push: ${CYAN}$FORCE${NC}"
    echo -e "${WHITE}Push Tags: ${CYAN}$PUSH_TAGS${NC}"
    echo -e "${WHITE}Push Time: ${CYAN}$(date '+%Y-%m-%d %H:%M:%S')${NC}"
}

# Main push process
main() {
    log_header "🚀 VERSIONER GIT PUSH SCRIPT 🚀"
