#!/usr/bin/env zsh
# Version artifacts script for macOS/Linux
# Based on _versionArtifacts.sh with enhanced reporting
...
show_help() {
    cat << EOF
Versioner Artifacts Versioning Script
...
run_versioner() {
    ...
    # Note: Working folder (-w) is required, log level (-l) uses single letter codes (V|D|I|W|E|F)
    local versioner_args=(
        "-w=$WORKING_DIR"
    )
    
    # Convert log level to single letter code if needed
    local log_level_code="I"
    case "$LOG_LEVEL" in
        Verbose|V) log_level_code="V" ;;
        Debug|D) log_level_code="D" ;;
        Information|Info|I) log_level_code="I" ;;
        Warning|W) log_level_code="W" ;;
        Error|E) log_level_code="E" ;;
        Fatal|F) log_level_code="F" ;;
    esac
    
    versioner_args+=("-l=$log_level_code")
    ...
    # Use Versioner from source code in current repository
    if [[ ! -f "$versioner_dir/Cli/Versioner.Cli.csproj" ]]; then
        log_error "Versioner.Cli.csproj not found in: $versioner_dir/Cli/"
        exit 1
    fi
    
    cd "$versioner_dir" || {
        log_error "Failed to change to Versioner directory: $versioner_dir"
        exit 1
    }
...
main() {
    log_header "🚀 VERSIONER ARTIFACTS VERSIONING SCRIPT 🚀"
