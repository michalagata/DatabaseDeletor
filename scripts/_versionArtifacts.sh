...
# Default to git root if in a git repository, otherwise current directory
if git rev-parse --git-dir > /dev/null 2>&1; then
    WORKING_DIR="${WORKING_DIR:-$(git rev-parse --show-toplevel)}"
else
    WORKING_DIR="${WORKING_DIR:-$(pwd)}"
fi
LOG_LEVEL="${LOG_LEVEL:-I}"
# Always store version file in repository root
...
OPTIONS:
    ...
    -s, --store-version       Store version in version.txt file [always enabled]
...
validate_environment() {
    ...
    # Ensure WORKING_DIR is set to git root for version.txt placement
    if [[ -z "${WORKING_DIR:-}" ]] || [[ "$WORKING_DIR" == "$(pwd)" ]]; then
        WORKING_DIR=$(git rev-parse --show-toplevel)
        log_info "Using git repository root as working directory: $WORKING_DIR"
    fi
    ...
    # Source maintainDotnetSDK.sh if available
    # shellcheck disable=SC1091
    if [[ -f "$SCRIPT_DIR/maintainDotnetSDK.sh" ]]; then
        source "$SCRIPT_DIR/maintainDotnetSDK.sh"
        if ! ensure_dotnet_sdk; then
            log_error "Cannot run Versioner - .NET SDK is required"
            exit 1
        fi
    elif ! command -v dotnet &> /dev/null; then
        log_error ".NET runtime not found. Please install .NET 10.0 or later."
        exit 1
    fi
...
run_versioner() {
    ...
    # Note: Working folder (-w) is required, log level (-l) defaults to V if not specified
    local versioner_args=(
        "-w=$WORKING_DIR"
    )
    
    # Add log level if specified (V|D|I|W|E|F)
    if [[ -n "$LOG_LEVEL" ]]; then
        versioner_args+=("-l=$LOG_LEVEL")
    fi
    ...
    # Add MonoRepo mode flag for global repository versioning
    versioner_args+=("--ismonorepo")
    
    # Add errors-as-warnings to handle optional configuration files
    versioner_args+=("--errors-as-warnings")
    ...
    if [[ -f "$versioner_dir/Cli/Versioner.Cli.csproj" ]]; then
        ...
        log_info "Using Versioner from source: $versioner_dir/Cli/Versioner.Cli.csproj"
        ...
        log_error "Versioner source not found at $versioner_dir/Cli/Versioner.Cli.csproj"
        exit 1
    fi
